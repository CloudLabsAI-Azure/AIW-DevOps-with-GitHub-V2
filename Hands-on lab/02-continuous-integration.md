...
In this exercise, you are going to set up the local infrastructure using dotnet. There are three parts of the application you will be working with: carts, products, and UI. You will deploy the infrastructure to the cloud using GitHub Actions. You will also build automation on GitHub for updating and republishing our workflows when the code changes.
...
1. You'll be able to see the lab files in Visual Studio code and explore the code files.
...
1. Open a **New Terminal** in Visual Studio Code by selecting, clicking on **menu (1)**, **Terminal (2)**, and then on **New Terminal (3)**.
...
1. Run the `dotnet user-secrets set "KeyVaultEndpoint" "https://contosotraderskv<SUFFIX>.vault.azure.net/"` command to set the secret path.
...
1. Keep the terminal running. Open a new browser tab and try accessing the application using the localhost port. You'll be able to see the output similar to the screenshot mentioned below.
...
1. Navigate back to **VS Code** and stop the terminal by typing **ctrl + C**. Run the below-mentioned command to navigate to the `ContosoTraders.Api.Carts` folder. 
...
1. Run the below-mentioned command to install npm.
   ...
   > **Note**: Please wait until the installation is completed. It will take around 10–15 minutes when you run npm install for the first time. In case the execution is stuck, please use **ctrl + C** to stop the execution and retry the step again.
   
1. Navigate back to **VS Code**. Run the below-mentioned command to navigate to the `ContosoTraders.Ui.Website` folder.
  
   ```pwsh
   cd C:\Workspaces\lab\aiw-devops-with-github-lab-files\src\ContosoTraders.Ui.Website
   ```

1. Now run the below-mentioned command to run the UI of the application. This will automatically open a browser tab where you'll see the complete application running
...
1. In a new browser tab, open ```https://www.github.com/login```. From the Environment Details page ***(1)***, navigate to the **License** ***(2)*** tab and **copy** ***(3)*** the credentials. Use the same username and password to login to GitHub.

   ![](media/dev2.png) 
   
1. For the **Device Verification Code**, use the same credentials as in the previous step, open `http://outlook.office.com/` in a private window, and enter the same username and password used for the GitHub Account login. Copy the verification code and paste it into Device Verification.
...
1. In Visual Studio Code, run the below commands in the terminal to set your **username** and **email**, which Git uses for commits. Make sure to replace the GitHub account email and username.
...
1. Select the **contoso-traders-<inject key="DeploymentID" enableCopy="false" />** resource group from the list.
...
1. Under the Settings side blade, select **Connection strings** ***(1)*** and copy the **ADO.NET (SQL authentication)** ***(2)*** connection string from ADO.NET tab. 
...
1. Under **Security**, expand **Secrets and variables** ***(1)*** by clicking the drop-down and select **Actions** ***(2)*** from the left navigation bar. Select the **New repository secret** ***(3)*** button.
...
1. Under **Actions Secrets/New secret** page, enter the below-mentioned details and click on **Add secret** ***(3)***.
   ...
   - **Value** : Paste the **ADO.NET (SQL authentication)** ***(2)*** which you copied in the previous step.
...
1. From the GitHub browser tab, follow the steps given below and click on **Create codespace on main** ***(3)***.
   ...
   >**Note**: In case you had created a codespace in a previous task, click on the **+** button to create a new codespace.
   
1. You'll be redirected to a new codespace tab in the browser. Please wait until the codespace is configured.

   ![](media/2dg33.png)
   
1. From the explorer side blade, navigate to **.github (1)** > **workflows** **(2)** and select the **contoso-traders-provisioning-deployment.yml** **(3)** file.
